...
**⚠️ Penting - Format Timestamp:**

Untuk `timestamp` dengan auto-update, gunakan format berikut:
...
- Gunakan constructor injection untuk `DatabaseManager`
- Definisikan schema lengkap untuk migration
- Buat helper methods untuk query yang sering digunakan
- Return type: `?array` (single), `array` (multiple), `int` (ID), `bool` (success/fail)
